...
const SearchBar = () => {
    ...
    const handleSubmit = (event: FormEvent) => {
        ...
        search.saveSearchValues(destination, checkIn, checkOut, childCount, adultCount);
        navigate("/search");
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="-mt-8 p-3 bg-orange-400 rounded shadow-md grid grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5 items-center gap-4">
            ...
            <div>
                <DatePicker selected={checkOut} onChange={(date) => setCheckOut(date as Date)} selectsStart startDate={checkIn} endDate={checkOut} minDate={minDate} maxDate={maxDate} placeholderText="Check-in Date" className="min-w-full bg-white p-2 focus:outline-none"/>
            </div>
